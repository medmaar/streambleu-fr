      ...
      <main style={{ background: "#E8F4F5", minHeight: "100vh" }}>
        ...
        <section style={{ background: "linear-gradient(to right, #1A3D45, #2d6a78)", padding: "80px 16px 60px" }}>
          <div style={{ maxWidth: 820, margin: "0 auto" }}>
            <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
              <span style={{ background: "#F96E5B", color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 12px", borderRadius: 999, textTransform: "uppercase" }}>Avis</span>
            ...
            <h1 style={{ fontSize: "clamp(28px, 4.5vw, 50px)", fontWeight: 900, color: "#fff", lineHeight: 1.15, marginBottom: 20 }}>
              ...
              <span style={{ color: "#F96E5B" }}>Services Testés & Classés</span>
            </h1>
        ...
        <article style={{ maxWidth: 820, margin: "0 auto", padding: "60px 16px" }}>
          ...
          <section style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: "#1A3D45", marginBottom: 16 }}>
          ...
          {/* Criteria */}
          <section style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: "#1A3D45", marginBottom: 20 }}>
              Nos critères d&apos;évaluation
            </h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
              {criteria.map((c, i) => (
                <div key={c.label} style={{ background: "#fff", borderRadius: 14, padding: "20px 20px", border: "1px solid rgba(63,154,174,0.15)" }}>
                  <div style={{ fontWeight: 800, fontSize: 24, color: "#F96E5B", marginBottom: 6 }}>{i + 1}.</div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: "#1A3D45", marginBottom: 6 }}>{c.label}</div>
          ...
          <section style={{ marginBottom: 48 }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {services.map((s, i) => (
                    <tr key={s.name} style={{ background: i === 0 ? "rgba(249,110,91,0.06)" : i % 2 === 0 ? "#fff" : "rgba(63,154,174,0.04)" }}>
                      <td style={{ padding: "12px 14px", fontWeight: 900, color: i === 0 ? "#F96E5B" : "#1A3D45", fontSize: 16 }}>#{s.rank}</td>
                      <td style={{ padding: "12px 14px", fontWeight: 700, color: "#1A3D45" }}>{s.name}</td>
                      <td style={{ padding: "12px 14px", color: "#333" }}>{s.channels}</td>
                      <td style={{ padding: "12px 14px", color: i === 0 ? "#3F9AAE" : "#333", fontWeight: i === 0 ? 700 : 400 }}>{s.quality}</td>
                      ...
                      <td style={{ padding: "12px 14px", color: i === 0 ? "#F96E5B" : "#555", fontWeight: i === 0 ? 700 : 400 }}>{s.verdict}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section style={{ marginBottom: 48 }}>
            ...
            <div style={{ background: "rgba(249,110,91,0.06)", border: "1px solid rgba(249,110,91,0.2)", borderRadius: 14, padding: "20px 24px", marginBottom: 20 }}>
              <p style={{ fontSize: 14, color: "#1A3D45", fontWeight: 700, marginBottom: 8 }}>✓ Ce que nous avons aimé</p>
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {["Vrai 4K H.265/HEVC avec HDR10", "25 000+ chaînes dont toute la TNT française", "Activation en moins de 30 minutes", "Support 24/7 en français par WhatsApp & email", "Essai gratuit 24h sans carte bancaire", "À partir de 9€/mois sans engagement"].map(p => (
                  <li key={p} style={{ fontSize: 14, color: "#333", marginBottom: 5, display: "flex", gap: 8 }}>
                    <span style={{ color: "#F96E5B", fontWeight: 700 }}>✓</span>{p}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {/* Final verdict */}
          <section style={{ background: "#1A3D45", borderRadius: 20, padding: "36px 32px", marginBottom: 48, textAlign: "center" }}>
            ...
            <Link href="/free-trial" style={{
              background: "#F96E5B", color: "#fff", fontWeight: 700, fontSize: 16,
