...
const rewardCards = [
  { title: "+1 An par Parrainage", desc: "Chaque parrainage approuvé ajoute 12 mois gratuits à votre abonnement.", bg: "#1A3D45" },
  { title: "Sans Limite", desc: "Parrainez autant de personnes que vous voulez. Chaque parrainage réussi rapporte une autre année bonus.", bg: "#3F9AAE" },
  { title: "Validation Rapide", desc: "Notre équipe examine et approuve manuellement les parrainages rapidement.", bg: "#F96E5B" },
];
    ...
    <main style={{ background: "linear-gradient(to left, #daf0f2 0%, #fce8e4 100%)", color: "#000000", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <div style={{ background: "#1A3D45", borderRadius: 24, padding: "28px 24px", boxShadow: "0 8px 32px rgba(26,61,69,0.25)", textAlign: "center" }}>
            ...
            <h1 style={{ fontSize: "clamp(2rem, 5vw, 3rem)", fontWeight: 900, color: "#fff", lineHeight: 1.1, marginBottom: 14 }}>
              Parrainez un ami,<br /><span style={{ color: "#F96E5B" }}>gagnez +1 an gratuit</span>
            </h1>
      ...
      <section style={{ padding: "48px 16px", background: "#fff" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <h2 style={{ textAlign: "center", fontSize: "clamp(1.6rem, 3vw, 2rem)", fontWeight: 900, color: "#1A3D45", marginBottom: 36 }}>
            Comment ça fonctionne
          </h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {steps.map((step) => (
              <div key={step.number} style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
                <div style={{ flexShrink: 0, width: 48, height: 48, background: "#F96E5B", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 16, color: "#fff" }}>
                  {step.number}
                </div>
                <div>
                  <h3 style={{ fontWeight: 700, fontSize: 16, color: "#1A3D45", marginBottom: 6 }}>{step.title}</h3>
      ...
      <section style={{ padding: "48px 16px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <h2 style={{ fontSize: "1.4rem", fontWeight: 800, color: "#1A3D45", marginBottom: 20 }}>Règles du programme</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {rules.map((rule, i) => (
              <div key={i} style={{ display: "flex", gap: 12, background: "#fff", borderRadius: 12, padding: "14px 18px", border: "1px solid rgba(63,154,174,0.15)" }}>
      ...
      {/* Formulaire */}
      <section style={{ padding: "48px 16px 80px", background: "#1A3D45" }}>
